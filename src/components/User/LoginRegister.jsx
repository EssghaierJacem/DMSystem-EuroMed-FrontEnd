...
function LoginRegister() {
    ...
    const handleRegisterSubmit = async (e) => {
        ...
        if (registerData.registerPassword !== registerData.confirmPassword) {
            setFieldErrors((prev) => ({ ...prev, password: "Les mots de passe ne correspondent pas." }));
            return;
        }

        if (!registerData.registerPassword.trim()) {
            setFieldErrors((prev) => ({ ...prev, password: "Le mot de passe ne peut pas être vide." }));
            return;
        }
        ...
        try {
            ...
            const { usernameExists, emailExists } = checkUserResponse.data;

            if (usernameExists) {
                setFieldErrors((prev) => ({ ...prev, userExists: "Cet utilisateur existe déjà." }));
                return;
            }

            if (emailExists) {
                setFieldErrors((prev) => ({ ...prev, emailExists: "Cet email existe déjà." }));
                return;
            }
                                ...
                                <div className="tab-content" id="myTabContent">
                                    ...
                                    >
                                        <form className="auth-form" onSubmit={handleRegisterSubmit}>
                                            <div className="mb-3 form-group">
                                                <i className="iconsax" data-icon="user"></i>
                                                <label htmlFor="registerNomUtilisateur" className="form-label">
                                                    Nom
                                                </label>
                                                <input
                                                    type="text"
                                                ...
                                                {fieldErrors.userExists && (
                                                    <p style={{ color: "red" }}>{fieldErrors.userExists}</p>
                                                )} {/* Display specific error message for existing users */}
                                            </div>
                                            <div className="mb-3 form-group">
                                                <i className="iconsax" data-icon="mail"></i>
